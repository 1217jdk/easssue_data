...
original_df = pd.read_csv(f'/root/Data/data/article/{article_filedate}_{article_try}_article.csv')
...
def desc_to_summary(description):
    ...
    ## 기사 본문 줄글 -> 문장 구분해주기
    description = kss.split_sentences(description.replace('굈굈', ' '))
    # print(description)
...
article_df.to_csv(f'/root/Data/data/article/{article_filedate}_{article_try}_article_db.csv', index=False, header=True)
