...
def news_clustering(df):
    ...
    ## [2] 임베딩 실패하면, 빈 것 return 하기
        ...
        print('에러발생', text)
        tmp_df = pd.DataFrame(
            {'title': [], 'link': [], 'pub_date': [], 'description': [], 'from_kwd_name': [], 'from_kwd_id': []})
        return tmp_df
    ...
    for clusterNum, indexList in clusterNum_indexList.items():
        ...
        for idx in indexList:
            cur_cosine = np.dot(embedded_description_vectors[idx], center) / (
                        norm(embedded_description_vectors[idx]) * center_norm)
...
def news_by_kwd(keyword_lst, processor_idx, return_dict, client_id, client_secret):
        ...
        if (rescode == 200):
            ...
            for item in items:
                ...
                if pubDate in target_times:
                    ...
                    cur_description.append(desc.replace('굈굈', ' '))
            ...
            if len(cur_title) > 3:
                ...
                cur_result_df = pd.DataFrame(
                    {'title': cur_title, 'link': cur_link, 'pub_date': cur_pub_date, 'description': cur_description,
                     'from_kwd_name': cur_from_kwd_name, 'from_kwd_id': cur_from_kwd_id})
    ...
    result_df = pd.DataFrame(
        {'title': title, 'link': link, 'pub_date': pub_date, 'description': description, 'from_kwd_name': from_kwd_name,
         'from_kwd_id': from_kwd_id})
...
def url_to_article(url):
    ...
    description = description.replace('굈굈', ' ')
...
def make_df_for_db(return_dict):
    ...
    result_df.loc[
        result_df['img'].str.contains(van), 'img'] = 'https://k7d102.p.ssafy.io/resource/default_article_img.png'
    ...
    result_df['summary'] = '.'
    result_df = result_df[
        ['article_id', 'category_id', 'title', 'link', 'pub_date', 'hit', 'summary', 'img', 'description',
         'from_kwd_name', 'from_kwd_id']]
    ...
    result_df.to_csv(f'/root/Data/data/article/{article_filedate}_{article_try}_article.csv', index=False, header=True)
    ...
    meta_df.to_csv('/root/Data/data/meta.csv', header=True, index=False)
...
client_df = pd.read_csv('/root/Data/data/naver_api.csv')
...
if __name__ == '__main__':
    ...
    for i in range(n):
        ...
        p = multiprocessing.Process(target=news_by_kwd, args=(
        kwd_lst[start:end], i, return_dict, client_id, client_secret))  ## 각 프로세스에 작업을 등록
        p.start()
        processes.append(p)
...
if __name__ == '__main__':
    ...
    for i in range(start1, end1):
        p = multiprocessing.Process(target=get_description_image,
                                    args=(return_dict[i], i, return_dict))  ## 각 프로세스에 작업을 등록
        p.start()
        processes.append(p)
