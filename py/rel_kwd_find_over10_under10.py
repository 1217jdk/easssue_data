...
## [2] keywords 가져오기
kwds = pd.read_csv(f'/root/Data/data/kwd/{kwd_filedate}_{kwd_try}_kwd.csv')
...
### (1) 전체와 DB용 둘다 가져오기
article_filename_lst = glob.glob(f'/root/Data/data/article/{two_days_ago}*.csv')
article_filename_lst += glob.glob(f'/root/Data/data/article/{one_days_ago}*.csv')
article_filename_lst += glob.glob(f'/root/Data/data/article/{today}*.csv')

article_filename_lst_subtract = glob.glob(f'/root/Data/data/article/{two_days_ago}*db.csv')
article_filename_lst_subtract += glob.glob(f'/root/Data/data/article/{one_days_ago}*db.csv')
article_filename_lst_subtract += glob.glob(f'/root/Data/data/article/{today}*db.csv')
...
over_10_article_df = over_10_article_df.sort_values(by='pub_date', ascending=False).groupby('from_kwd_name',
                                                                                            sort=False).head(
    10)  # DF의 GROUPBY해도 순서는 유지됨
print('over_10_article_df의 형태 ')
print(over_10_article_df.head())

## [3] 생성물 저장하기
over_10_article_df.to_csv(f'/root/Data/data/rel_kwd/{today}_over_10_article_df.csv', index=False, header=True)
under_10_kwd_df = pd.DataFrame({'under_10_kwd_lst': under_10_kwd_lst})
under_10_kwd_df.to_csv(f'/root/Data/data/rel_kwd/{today}_under_10_kwd_df.csv', index=False, header=True)
