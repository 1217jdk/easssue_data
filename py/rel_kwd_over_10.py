...
def over_10_rel_kwd_search(partial_over_10_kwd_lst, processor_idx,
                           return_dict):  # over_10_article_df, kwd_lst 이 만들어져 있어야 함
    ...
    for from_kwd_name in tqdm(partial_over_10_kwd_lst):
        ...
        from_kwd_name_full_description = over_10_article_df[over_10_article_df['from_kwd_name'] == from_kwd_name][
            'description']
    ...
    result_df = pd.DataFrame(
        {'from_kwd_id': result_from_kwd_id, 'to_kwd_id': result_to_kwd_id, 'to_kwd_count': result_to_kwd_count})
...
### (2) 데이터 가져오기
over_10_article_df = pd.read_csv(f'/root/Data/data/rel_kwd/{today}_over_10_article_df.csv')
...
if __name__ == '__main__':
    ...
    for i in range(n):
        ...
        p = multiprocessing.Process(target=over_10_rel_kwd_search, args=(
        over_10_kwd_lst[start:end], i, return_dict_over_10))  ## 각 프로세스에 작업을 등록 ## 인자로 db에 없는 kwd만 넣으면 됨
        p.start()
        processes.append(p)
...
## [3] 확인용
rel_kwd_df.to_csv(f'/root/Data/data/rel_kwd/{today}_rel_kwd_over_10.csv', index=False, header=True)
