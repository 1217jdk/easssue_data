...
def get_count(under_10_from_kwd_name_lst, under_10_full_description_lst, under_10_nouns_lst, processor_idx,
              return_dict):
...
### (2) 데이터 가져오기
under_10_kwd_df = pd.read_csv(f'/root/Data/data/rel_kwd/{today}_under_10_kwd_df.csv')
...
if __name__ == '__main__':
    ...
    for i in tqdm(range(n)):
        ...
        p = multiprocessing.Process(target=news_kwds_by_kwd, args=(
        under_10_kwd_lst[start:end], i, return_dict_under_10, client_id,
        client_secret))  ## 각 프로세스에 작업을 등록 ## 인자로 db에 없는 kwd만 넣으면 됨
        p.start()
        processes.append(p)
...
if __name__ == '__main__':
    ...
    for i in tqdm(range(n)):
        ...
        p = multiprocessing.Process(target=get_count, args=(
        under_10_from_kwd_name_lst[start:end], under_10_full_description_lst[start:end], under_10_nouns_lst[start:end],
        i, return_dict_under_10))
        p.start()
        processes.append(p)
...
rel_kwd_df_over_10 = pd.read_csv(f'/root/Data/data/rel_kwd/{today}_rel_kwd_over_10.csv')
...
## [5] 로컬에 저장하기
rel_kwd_df.to_csv(f'/root/Data/data/rel_kwd/{today}_rel_kwd.csv', index=False, header=True)
